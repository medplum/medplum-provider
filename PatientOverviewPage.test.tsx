...
import { ADMISSION_ENCOUNTER_KEY, SCREENING_ID_SYSTEM } from '../screeningData';
...
describe('PatientOverviewPage', () => {
  ...
  describe('admission Encounter link (task 40)', () => {
    // Task 40: the "Visits" tab already listed this Encounter correctly, but
    // nothing pointed at it — these prove the link only appears once there's
    // an actual admission Encounter to send someone to, and that it targets
    // the real Encounter's own id (not just the patient, or a guessed path).
    test('links directly to the admission Encounter when one exists', async () => {
      const encounter = await medplum.createResource({
        resourceType: 'Encounter',
        status: 'in-progress',
        class: { system: 'http://terminology.hl7.org/CodeSystem/v3-ActCode', code: 'IMP', display: 'inpatient encounter' },
        subject,
        identifier: screeningId(ADMISSION_ENCOUNTER_KEY),
      } as Resource);

      await renderOverview(medplum, patient.id);

      const link = await waitFor(() => screen.getByRole('link', { name: /view admission visit record/i }));
      expect(link).toHaveAttribute('href', `/Patient/${patient.id}/Encounter/${encounter.id}`);
    });

    test('does not render the link when there is no admission Encounter yet', async () => {
      await renderOverview(medplum, patient.id);

      await waitFor(() => expect(screen.getByText('Demographics')).toBeInTheDocument());
      expect(screen.queryByRole('link', { name: /view admission visit record/i })).not.toBeInTheDocument();
    });
  });
});
